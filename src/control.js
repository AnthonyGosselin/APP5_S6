...
var acceptedAdresses = ["093F5195-59F9-45F5-92D4-F287C0C9E31A"]
		...
		switch (data.name) {
			case "beaconEvent":
				...
				if (lastScannedUUID == data.data && (Math.floor(Date.now()/1000) - lastScannedTime <= 5)){
					console.log("[CONTROL] Uuid multiple scans: ", data.data)
					return
				}
				else if (acceptedAdresses.indexOf(data.data) == -1){
					console.log("[CONTROL] Uuid not authorized: ", data.data)
					return
				}
